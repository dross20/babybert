...
class AttentionHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        # The size of each attention head's latent space is equal to the embedding space's size divided by the number of heads.
        # That way, after we compute the context vector for each attention head, we can concatenate them to produce a vector
        # that's the same size as the original embedding space.
        head_size = config.hidden_size // config.n_heads

        # Layers for projecting the input embeddings into three spaces - the query (Q) space, the key (K) space, and the value (V) space.
    ...
    def forward(self, x, mask=None):
        ...
        # Perform scaled dot product attention (SDPA) using the query and key matrices - this yields the similarity between
        # each embedding in our input sequences.
        raw_attention_weights = (q @ torch.transpose(k, -2, -1)) / math.sqrt(q.size(-1))

        # If we have an attention mask, we replace each of the masked tokens with a negative infinity in our attention weight matrix.
        # This makes sure that the attention score will be 0 when we compute the softmax later.
        if mask is not None:
        ...
        # Taking the dot product of the weight matrix and the value matrix gives us our final context vector.
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Initialize n attention heads - this allows us to capture n sets of relationships between token embeddings.
...
class BabyBERT(nn.Module):
    """Minimal implementation of the [BERT](https://arxiv.org/pdf/1810.04805) architecture."""
    def __init__(self, config):
        super().__init__()
        ...
        # Having positional embeddings is necessary for the model to understand the order of the input tokens.
    ...
    def forward(self, x, mask=None):
        # Get the position IDs - these range from 0 to the max length of an input sequence (stored in the block_size variable).
        position_ids = torch.arange(0, self.config.block_size, device=x.device).unsqueeze(0)

        # Find the location of the [SEP] token. If there's just one segment in the input, [SEP] will occur at the end of the input.
        sep_token_mask = x == self.config.sep_token_id
        sep_token_position = sep_token_mask.int().argmax(dim=1).unsqueeze(1)

        # All positions leading up to and including the [SEP] token belong to segment 0; the rest belong to segment 1.
        segment_encodings = (position_ids > sep_token_position).long()

        # Sum the token, segment, and positional embeddings to obtain the complete embedding tensor.
